...
**El hito NO se cumple todavía.** De los doce módulos del frontend, tres consumen la
API REST y nueve siguen leyendo y escribiendo en `window.db` (localStorage).
...
| `login` | `window.db` ×3 | Kenner (SW-10) | ⛔ Pendiente — la API de sesión ya existe |
...
1. **`login`** (Kenner) — cambiar a `POST /api/auth/login` y cookie de sesión. Bloquea a todos.
